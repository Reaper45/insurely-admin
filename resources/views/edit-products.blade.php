...
<x-header >
    ...
    <div class="relative" >
         <form action="{{ route("products.delete", ["id" => $product->id, "_method" => "DELETE"]) }}" method="post">
            @csrf
            <button type="submit" class="inline-flex items-center px-2 py-1 border border-red-300 text-sm leading-5 font-medium rounded-md text-red-700 bg-red hover:text-red-500 focus:outline-none focus:shadow-outline-red focus:border-red-300 active:text-red-800 active:bg-red-50 transition duration-150 ease-in-out">
                Delete
            </button>
        </form>
    </div>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach ($product->charges as $charge)
                                <tr>
                                    <td class="px-6 py-4 whitespace-no-wrap">
                                        <div class="text-sm leading-5 text-gray-900">{{ $charge->name }}</div>
                                        {{-- <div class="text-sm leading-5 text-gray-500">{{ $charges->limit }}</div> --}}
                                    </td>
                                    <td class="px-6">
                                        <div class="text-xs leading-5 text-gray-500">
                                            {{ $charge->is_percentage ? "" : "Ksh. " }}
